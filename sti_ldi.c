					...
					unsigned char *codage)
{
	...
	*codage = map[(proc->pc + 1) % MEM_SIZE];
	if (g_tab[op_index].is_codage && !check_codage(*codage, proc, op_index))
	{
		proc->inst_cycle = 0;
		return (0);
	}
...
//	print_map(map);
	proc->inst_cycle = 0;
	if (ft_strequ(g_tab[op_index].name, "ld") ||
			ft_strequ(g_tab[op_index].name, "ldi"))
	{
		...
		if ((heap_args = take_argument(map, codage, proc, op_index, 1)) == NULL)
			return;
	parse_heap_to_stack_args(arg, &heap_args, g_tab[op_index].count_arg);
	if (ft_strequ(g_tab[op_index].name, "ldi") ||
			ft_strequ(g_tab[op_index].name, "lldi"))
	{
	...
	if (ft_strequ(g_tab[op_index].name, "ldi"))
	...
	else if (ft_strequ(g_tab[op_index].name, "lldi"))
		load_value =
		...
		load_value = arg[0];
	if (ft_strequ(g_tab[op_index].name, "ld") ||
			ft_strequ(g_tab[op_index].name, "lld"))
		proc->regs[arg[1]] = load_value;
	else if (ft_strequ(g_tab[op_index].name, "ldi") ||
			 ft_strequ(g_tab[op_index].name, "lldi"))
...
void	ft_st_sti(t_bs *bs, t_proc *proc, unsigned short op_index)
{
	...
	if (for_instruct(bs->map, proc, op_index, &codage) == 0)
		return ;
	proc->inst_cycle = 0;
	if (ft_strequ(g_tab[op_index].name, "st") && codage == 0x70)
	{
		codage = 0x60;
	...
	arg[0] = (((codage >> 6) & 0x3) == REG_CODE) ?
			 proc->regs[arg[0]] : arg[0];
	if (ft_strequ(g_tab[op_index].name, "st") && codage == 0x50)
	{
	...
	arg[1] = (((codage >> 4) & 0x3) == REG_CODE) ?
			 proc->regs[arg[1]] : arg[1];
	if (ft_strequ(g_tab[op_index].name, "sti"))
		arg[2] = (((codage >> 2) & 0x3) == REG_CODE) ?
				 proc->regs[arg[2]] : arg[2];
	if (ft_strequ(g_tab[op_index].name, "st"))
	{
	...
	else if (ft_strequ(g_tab[op_index].name, "sti"))
		i = (unsigned int)
				   ...
				   unsigned short op_index)
{
	...
	tmp->inst_cycle = 0;
	codage = DIR_CODE << 6;
	if (ft_strequ(g_tab[op_index].name, "fork"))
	{
	...
	if (ft_strequ(g_tab[op_index].name, "fork"))
		new_proc->pc = (unsigned int)((long)tmp->pc + (short)arg[0] % IDX_MOD < 0 ?
						...
						((long)tmp->pc + (short)arg[0] % IDX_MOD) % MEM_SIZE);
	else if (ft_strequ(g_tab[op_index].name, "lfork"))
