...
void    check_inst(t_proc *procs, unsigned char map[MEM_SIZE])
{
    t_proc *tmp;
    ...
    while (tmp)
    {
        if (tmp->live && tmp->cycle_to_die != CYCLE_TO_DIE)
//TODO в кожній інструкції перевіряти inst_cycle, якщо він дорівнює циклу інструкцій то дана інструкція виконується в іншому разі inst_cycle збільшується на 1
            if (tmp->pc == op_tab[0].opcode)
                ;//TODO функція live
            else if (tmp->pc == op_tab[1].opcode)
                ;//TODO функція ld
            else if (tmp->pc == op_tab[2].opcode)
                ;//TODO функція st
            else if (tmp->pc == op_tab[3].opcode)
                ;//TODO функція add
            else if (tmp->pc == op_tab[4].opcode)
                ;//TODO функція sub
            else if (tmp->pc == op_tab[5].opcode)
                ;//TODO функція and
            else if (tmp->pc == op_tab[6].opcode)
                ;//TODO функція or
            else if (tmp->pc == op_tab[7].opcode)
                ;//TODO функція xor
            else if (tmp->pc == op_tab[8].opcode)
                ;//TODO функція zjmp
            else if (tmp->pc == op_tab[9].opcode)
                ;//TODO функція ldi
            else if (tmp->pc == op_tab[10].opcode)
                ;//TODO функція sti
            else if (tmp->pc == op_tab[11].opcode)
                ;//TODO функція fork
            else if (tmp->pc == op_tab[12].opcode)
                ;//TODO функція lld
            else if (tmp->pc == op_tab[13].opcode)
                ;//TODO функція lldi
            else if (tmp->pc == op_tab[14].opcode)
                ;//TODO функція lfork
            else if (tmp->pc == op_tab[15].opcode)
                ;//TODO функція aff
        tmp = tmp->next;
    }
}
...
void	global_cycles(t_bs *bs)
{
	...
	{
		check_inst(bs->list_proc, bs->map);
